...
# Perform optimization with SLSQP
yaw_angles = yaw_opt.optimize()

# Perform optimization with serial refine
yaw_angles_sr = serial_refine(fi, max_yaw=max_yaw, allow_negative_yaw=False)

print("==========================================")
print("yaw angles = ")
for i in range(len(yaw_angles)):
    print("(SLSQP) Turbine ", i, "=", yaw_angles[i], " deg")
    print("(Serial Refine) Turbine ", i, "=", yaw_angles_sr[i], " deg")

# Assign yaw angles to turbines and calculate wake
fi.calculate_wake(yaw_angles=yaw_angles)
power_opt_slsqp = fi.get_farm_power()
fi.calculate_wake(yaw_angles=yaw_angles_sr)
power_opt_sr = fi.get_farm_power()

print("==========================================")
print(
    "Total Power Gain (SLSQP) = %.1f%%"
    % (100.0 * (power_opt_slsqp - power_initial) / power_initial)
)
print(
    "Total Power Gain (SR) = %.1f%%"
    % (100.0 * (power_opt_sr - power_initial) / power_initial)
)
print("==========================================")
